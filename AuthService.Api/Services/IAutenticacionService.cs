...
namespace AuthService.Api.Services
{
    ...
    public interface IAutenticacionService
    {
        ...
        Task<IResult> ResetPasswordAsync(ResetPasswordRequest request, string? ip, string? userAgent);
        Task<IResult> ConfirmPasswordChangeAsync(string token, string? ip, string? userAgent);
        Task<IResult> RefreshTokenAsync(RefreshTokenRequest request, string? userAgent, string? ip);
        Task<IResult> ChangePasswordAsync(ChangePasswordRequest request, long idUsuario, string? ip, string? userAgent);
        Task<IResult> LogoutAsync(long idSesion);
        Task<IResult> LogoutAllAsync(long idUsuario, string? ip, string? userAgent);
        Task<IResult> GetSessionsAsync(long idUsuario);
        Task<IResult> RevokeSessionAsync(long idSesion, long idUsuario, string? ip, string? userAgent);
